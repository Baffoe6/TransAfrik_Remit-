...
import { CustomerQuoteSummary } from "./CustomerQuoteSummary";
    ...
    <FintechCard variant={compact ? "elevated" : "hero"}>
      {!compact && (
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: spacing.md }}>
          <Text style={[typography.label, { color: "rgba(255,255,255,0.75)" }]}>Your quote</Text>
      ...
      {quote && !isError ? (
        <>
          ...
          <CustomerQuoteSummary quote={quote} currency={currency} light={!compact} compact={compact} />
          {partner && (
        ...
        <Text style={{ color: compact ? theme.error : "#FCA5A5", fontSize: 13 }}>Unable to fetch live quote</Text>
      ) : (
