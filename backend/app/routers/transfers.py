...
from app.models.enums import BeneficiaryStatus, CancellationReason, KycStatus, PaymentProofStatus, TransferStatus
...
from app.services.pricing import build_customer_quote, calculate_transfer_amounts
from app.services.transfer_cancellation_service import cancel_transfer, can_customer_cancel
...
TRANSFER_AMOUNT_FIELDS = frozenset({
    "send_amount_zar",
    "fee_zar",
    "exchange_rate",
    "receive_amount_ghs",
...
def calculate(data: CalculatorRequest, db: Annotated[Session, Depends(get_db)]):
    try:
        amounts = calculate_transfer_amounts(
            db,
            data.amount_to_pay_zar,
            destination_country=data.destination_country,
            payment_method_id=data.payment_method_id,
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    return CalculatorResponse(**build_customer_quote(amounts))
...
def create_transfer(
    ...
    check_transfer_limits(db, current_user.id, data.amount_to_pay_zar, beneficiary)
    ...
    compliance = run_transfer_compliance(
        ...
        send_amount_zar=amounts["total_amount_zar"],
...
@router.post("/{transfer_id}/cancel", response_model=TransferDetailResponse)
def cancel_transfer_customer(
    transfer_id: int,
    current_user: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)],
):
    ...
    if not can_customer_cancel(transfer):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Only unpaid transfers can be cancelled",
        )
